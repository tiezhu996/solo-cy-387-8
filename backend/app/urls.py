from django.urls import path

from app.apps.booking.views import BookingCreateView
from app.apps.contract.views import (
    ContractCancelView,
    ContractConfirmView,
    ContractDetailView,
    ContractListView,
    ContractSettlementView,
    ContractTerminateView,
)
from app.apps.properties.views import PropertyListView
from app.apps.repair.views import RepairTicketView

urlpatterns = [
    path('api/properties/', PropertyListView.as_view()),
    path('api/bookings/', BookingCreateView.as_view()),
    path('api/contracts/', ContractListView.as_view()),
    path('api/contracts/<int:contract_id>/', ContractDetailView.as_view()),
    path('api/contracts/<int:contract_id>/confirm/', ContractConfirmView.as_view()),
    path('api/contracts/<int:contract_id>/cancel/', ContractCancelView.as_view()),
    path('api/contracts/<int:contract_id>/terminate/', ContractTerminateView.as_view()),
    path('api/contracts/<int:contract_id>/settlement/', ContractSettlementView.as_view()),
    path('api/repairs/', RepairTicketView.as_view()),
]
