...
class ContractConfirmTests(APITestCase):
    ...
    def test_second_pending_contract_confirm_rejected(self):
        # 同一房源再发起一份租期不重叠的待确认合同
        resp = self.client.post(
            '/api/contracts/',
            contract_payload(self.property.id, startDate='2026-07-02', endDate='2026-12-31'),
            format='json',
        ...
        self.client.post(f'/api/contracts/{self.contract_id}/confirm/')
        resp = self.client.post(f'/api/contracts/{second_id}/confirm/')
        self.assertEqual(resp.status_code, 400)
        self.assertEqual(resp.json()['error']['code'], 'CONTRACT_DUPLICATE')
        self.assertEqual(resp.json()['error']['message'], '该房源已存在生效合同，不能重复签署')

        # 失败请求不得改动合同、房源或历史数据
        second = Contract.objects.get(id=second_id)
        self.assertEqual(second.status, CONTRACT_STATUS_PENDING)
    ...
    def test_confirm_allowed_after_previous_terminated(self):
        resp = self.client.post(
            '/api/contracts/',
            contract_payload(self.property.id, startDate='2026-07-02', endDate='2026-12-31'),
            format='json',
        ...
        resp = self.client.post(f'/api/contracts/{second_id}/confirm/')
        self.assertEqual(resp.status_code, 200)
        self.assertEqual(resp.json()['status'], CONTRACT_STATUS_ACTIVE)
        self.property.refresh_from_db()
        self.assertEqual(self.property.status, HOUSE_STATUS_SIGNED)
